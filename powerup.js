...
TrelloPowerUp.initialize({
 ...
 'card-detail-badges':function(t,opts){
   return t.get('card','shared','timeData',{plannedMinutes:0,spentMinutes:0,timerRunning:false,timerStart:0,history:[]}).then(function(d){
     var planned=d.plannedMinutes||0,spent=d.spentMinutes||0,running=d.timerRunning||false,badges=[];
     ...
     if(!running){
       ...
       badges.push({text:'Stop',icon:ICON,callback:function(t){
         return t.get('card','shared','timeData',{plannedMinutes:0,spentMinutes:0,timerRunning:false,timerStart:0,history:[]}).then(function(data){
           ...
           return askForNote(t,'').then(function(note){data.history.push({type:'stop',minutes:diffMin,ts:new Date().toISOString(),note:note||''});return t.set('card','shared','timeData',data).then(function(){if(typeof t.comment==='function'){return t.comment('Automatycznie dodano '+diffMin+' minut po zatrzymaniu timera.').catch(function(){});}});});
         }).then(function(){t.refresh();});}});
     }
     return badges;
   });
 ...
 'show-settings':function(t){return t.popup({title:'Time Tracker NM - Ustawienia',url:'settings.html',height:420});}
});